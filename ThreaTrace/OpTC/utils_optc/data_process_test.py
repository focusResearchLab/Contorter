...
def MyDatasetA(path, dataset_name):
	# graphId = model
	feature_num = 0
	label_num = 0
	f_feature = open(f'../models/{dataset_name}/feature.txt', 'r')
	feature_map = {}
	for i in f_feature:
		temp = i.strip('\n').split('\t')
	...
	f_label = open(f'../models/{dataset_name}/label.txt', 'r')
	label_map = {}
	for i in f_label:
		temp = i.strip('\n').split('\t')
	...
	f_gt = open(f'../groundtruth/{dataset_name}/groundtruth_uuid.txt', 'r')
	ground_truth = {}
	for line in f_gt:
	...
	adj2 = {}
	data_thre = 1000000
	fw = open(f'../groundtruth/{dataset_name}/groundtruth_nodeId.txt', 'w')
	fw2 = open(f'../groundtruth/{dataset_name}/id_to_uuid.txt', 'w')
	nodeId_map = {}
	cnt = 0
	nodeA = []
	for i in range(1):
	...
	return [data1], feature_num, label_num, adj, adj2, nodeA